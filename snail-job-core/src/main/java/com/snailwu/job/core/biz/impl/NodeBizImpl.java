...
public class NodeBizImpl implements NodeBiz {
    ...
    public ResultT<String> run(TriggerParam triggerParam) {
        ...
        IJobHandler newJobHandler = JobExecutor.loadJobHandler(executorHandler);
        ...
        if (jobThread == null) { // 该任务第一次被调度
            ...
            jobThread = JobExecutor.registryJobThread(jobId, newJobHandler, null);
        } else { // 任务非第一次被调度
            ...
            if (jobHandler != newJobHandler) {
                ...
                jobThread = JobExecutor.registryJobThread(jobId, jobHandler, "任务对应的JobHandler发生变化");
            }
        }
