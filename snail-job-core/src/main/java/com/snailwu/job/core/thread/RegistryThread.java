...
public class RegistryThread {
    public static final Logger LOGGER = LoggerFactory.getLogger(RegistryThread.class);
        ...
        registryThread = new Thread(() -> {
            while (!stopFlag) {
                ...
                try {
                    ...
                    ResultT<String> result = JobExecutor.getAdminBiz().registry(registryParam);
            ...
            try {
                RegistryParam registryParam = new RegistryParam(groupName, address);
                ResultT<String> result = JobExecutor.getAdminBiz().registryRemove(registryParam);
                if (ResultT.SUCCESS_CODE != result.getCode()) {
