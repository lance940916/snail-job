...
public class NodeBizImpl implements NodeBiz {
    ...
    public ResultT<String> idleBeat(IdleBeatParam idleBeatParam) {
        JobThread jobThread = SnailJobNode.loadJobThread(idleBeatParam.getJobId());
    ...
    public ResultT<String> run(TriggerParam triggerParam) {
        ...
        // 通过 jobId, 获取执行线程
        JobThread jobThread = SnailJobNode.loadJobThread(jobId);
        if (jobThread == null) { // 该任务第一次被调度
            // 生成 Job 对应的 jobThread
            jobThread = SnailJobNode.registryJobThread(jobId, newJobHandler, null);
        } else { // 任务非第一次被调度
            ...
            if (jobHandler != newJobHandler) {
                ...
                jobThread = SnailJobNode.registryJobThread(jobId, jobHandler, "任务对应的JobHandler发生变化");
            }
        }
    ...
    public ResultT<String> kill(KillParam killParam) {
        // 获取调度线程
        JobThread jobThread = SnailJobNode.loadJobThread(killParam.getJobId());
        if (jobThread == null) {
